...
Функция Создать_SALE_ФайлDBF(ИмяФайла, НачалоПериода = Неопределено, КонецПериода = Неопределено) Экспорт		
	...
	Пока ТЗ.Следующий() Цикл 
		...
		Попытка
			...
			дбф.CODE		= ТЗ.Код;
			//Если ТЗ.Код="86917" ИЛИ ТЗ.Код="86788" ИЛИ ТЗ.Код="86790" ИЛИ ТЗ.Код="86789" ИЛИ ТЗ.Код="136249" ИЛИ ТЗ.Код="136250" Тогда 
			Если РаботаСПС.ЭтоПодарочныйСертификат(дбф.CODE) Тогда
			...
			КонецЕсли;	
			дбф.Записать();		
			Логирование.ДобавитьЗаписьЖурнала(УровеньЖурналаРегистрации.Информация, "Создать_SALE_ФайлDBF()", "Файл успешно создан: " + ИмяФайла, Неопределено, Неопределено, "ОбщийМодуль.ЗакрытиеСмены");	 	
		Исключение
			Логирование.ДобавитьЗаписьЖурнала(УровеньЖурналаРегистрации.Ошибка, "Создать_SALE_ФайлDBF()", "Файл не создан: " + ИмяФайла, Неопределено, Неопределено, "ОбщийМодуль.ЗакрытиеСмены");	 	
		КонецПопытки;	
	КонецЦикла;
...
Функция Создать_а_SALE_ФайлDBF(ИмяФайла, НачалоПериода = Неопределено, КонецПериода = Неопределено) экспорт		
	...
	Пока ТЗ.Следующий() Цикл 
		...
		дбф.CODE     														= ТЗ.Код;
		//Если ТЗ.Код="86917" ИЛИ ТЗ.Код="86788" ИЛИ ТЗ.Код="86790" ИЛИ ТЗ.Код="86789" ИЛИ ТЗ.Код="136249" ИЛИ ТЗ.Код="136250"  Тогда  
		Если РаботаСПС.ЭтоПодарочныйСертификат(ТЗ.Код) Тогда
...
Функция Создать_Z_Sale(СтруктураПараметровКассы) Экспорт
	...
	Если НЕ ПустаяСтрока(Результат) тогда
  	...
  	Иначе 
		...
		Логирование.ДобавитьЗаписьЖурнала(УровеньЖурналаРегистрации.Ошибка, "Создать_Z_Sale()", "Файл не создан: " + ИмяФайла, Неопределено, Неопределено, "ОбщийМодуль.ЗакрытиеСмены");	 	
	КонецЕсли;	
...
Процедура Создать_а_Sale(СтруктураПараметровКассы) Экспорт
	...
	Иначе 
		Логирование.ДобавитьЗаписьЖурнала(УровеньЖурналаРегистрации.Ошибка, "Создать_а_Sale()", "Файл не создан: " + ИмяФайла, Неопределено, Неопределено, "ОбщийМодуль.ЗакрытиеСмены");	 	
	КонецЕсли;
...
Процедура Создать_NoCheck(СтруктураПараметровКассы) Экспорт
	...
	Если НЕ ПустаяСтрока(Результат) тогда
    	...
    	НоваяСтрока.ИмяПодкаталога 	= "";
 		НоваяСтрока.ПерезаписьНаFTP = Истина;	
	Иначе 
		Логирование.ДобавитьЗаписьЖурнала(УровеньЖурналаРегистрации.Ошибка, "Создать_NoCheck()", "Файл не создан: " + ИмяФайла, Неопределено, Неопределено, "ОбщийМодуль.ЗакрытиеСмены");	 	
	КонецЕсли;	
...
Функция ЗаписатьПродажи_В_РС_ЖурналРРО() Экспорт
	...
	Если Таблица.Количество() = 0 Тогда 
		...
		Логирование.ДобавитьЗаписьЖурнала(, "ЗаписатьПродажи_В_РС_ЖурналРРО()", "ТекущийZОтчет_Обновлен_В_ЖурналРРО", Неопределено, Неопределено, "ОбщийМодуль.ЗакрытиеСмены");	 
	Иначе
		Попытка
			...
			Логирование.ДобавитьЗаписьЖурнала(, "ЗаписатьПродажи_В_РС_ЖурналРРО()", "ТекущийZОтчет_Обновлен_В_ЖурналРРО", Неопределено, Неопределено, "ОбщийМодуль.ЗакрытиеСмены");	 
		Исключение
			Логирование.ДобавитьЗаписьЖурнала(УровеньЖурналаРегистрации.Ошибка, "ЗаписатьПродажи_В_РС_ЖурналРРО()", "ТекущийZОтчет_НЕ_ЗАПИСАН_В_ЖурналРРО", Неопределено, Неопределено, "ОбщийМодуль.ЗакрытиеСмены");	 
