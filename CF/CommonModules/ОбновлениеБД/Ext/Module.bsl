...
Процедура ВыполнитьОбновление() Экспорт
	...
	Если НЕ Результат.УстановкаОбновлений Тогда  		Возврат;	КонецЕсли;	
	...
	Попытка                                                                        
		...
		шелл.Run(ПроверитьПараметрНаПробелы(КаталогПрограммы()+"1CV8.EXE")+ " CONFIG /F " + ПроверитьПараметрНаПробелы(КаталогИБ()) + " /N Программист /P developer13 /UpdateCfg " + ПроверитьПараметрНаПробелы(КаталогВременныхФайлов() + Результат.ИмяФайлаОбновлений),1,1);
