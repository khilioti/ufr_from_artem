...
Процедура ПараметрыФР_Очистить(ПараметрыФР) Экспорт 
	глЭККР																	= Неопределено;
...
Функция ПротеститьПодключениеФР(ПараметрыФР) Экспорт  
	...
	Пока КоличествоПопыток > 0 Цикл
		Если ПолучитьОбработкуТО().ПроверитьПодключениеФР(ПараметрыФР) Тогда 	Прервать;	КонецЕсли;
		...
		Попытка
			глЭККР       													= ПолучитьОбработкуТО().ЗакрытьПорт(ПараметрыФР);
	 		глЭККР       													= ПолучитьОбработкуТО().ОткрытьПорт(ПараметрыФР);  // ++VS 02.10.2013 Инициализация COM-объекта подключаемого РРО
			ПолучитьОбработкуТО().ПолнаяАннуляцияЧека(глЭККР);    		// ++VS 31.08.2014 Анулируем зависшие чеки в ФР
			ПолучитьОбработкуТО().УстановитьОператора(ПараметрыФР);    	// ++VS 09.09.2013 установка оператора для печати на чеке
		Исключение
...
Функция ПереОткрытьПортЭККР() Экспорт 
	...
	Пока глЭККР = Неопределено И КоличествоПопыток > 0 Цикл  
		Попытка
			...
			ПолучитьОбработкуТО().ПолнаяАннуляцияЧека(глЭККР);    		// ++VS 31.08.2014 Анулируем зависшие чеки в ФР
			ПолучитьОбработкуТО().УстановитьОператора(ПараметрыФР);    	// ++VS 09.09.2013 установка оператора для печати на чеке
			глСерийныйНомерФР_VS = ПолучитьОбработкуТО().ПолучитьСерийныйНомер(ПараметрыФР);
...
Процедура ИнициализироватьФР(ПараметрыФР, ПараметрыЧекаККМ, ЕстьАварийныйЧекТерминал) Экспорт
	...
	Пока глЭККР = Неопределено И КоличествоПопыток > 0 Цикл  
		Попытка
			...
			ПолучитьОбработкуТО().ПолнаяАннуляцияЧека(глЭККР);    					// ++VS 31.08.2014 Анулируем зависшие чеки в ФР
			ПолучитьОбработкуТО().УстановитьОператора(ПараметрыФР);    				// ++VS 09.09.2013 установка оператора для печати на чеке
            ПолучитьОбработкуТО().ПолучитьНомерZОтчета_ТекущейСмены(ПараметрыФР);	// ++VS 02.10.2013 Получаем номер текущего Z-отчета
			глСерийныйНомерФР_VS = ПолучитьОбработкуТО().ПолучитьСерийныйНомер(ПараметрыФР);
	...
	Если НЕ глЭККР = Неопределено Тогда 					
		...
		Пока Истина Цикл    
			Попытка
				...
				ДатаНаФР = ПолучитьОбработкуТО().ПолучитьДатуВремяФР(ПараметрыФР);
			Исключение
				ДатаНаФР = Дата(1,1,1);
			КонецПопытки;	
		...
		Попытка
			ПолучитьОбработкуТО().ПолучитьФискальныйНомер(ПараметрыФР);
...
Функция ПроверитьОборотыПоФР(ПараметрыФР, ПараметрыЧекаККМ) Экспорт
	...
	Пока НЕ Результат_ПроверкаОборотовФР И КоличествоПопыток>0 Цикл 
		...
		Попытка
			глЭККР															= ПолучитьОбработкуТО().ЗакрытьПорт(ПараметрыФР); 
			глЭККР															= ПолучитьОбработкуТО().ОткрытьПорт(ПараметрыФР); 	
			Если ПолучитьОбработкуТО().ПроверитьПодключениеФР(ПараметрыФР) Тогда
...
//Процедура ЗаписатьОборотыФРВРегистрПередZОтчетом() Экспорт
//	
//	ОборотыФР = ПолучитьОбработкуТО().ОборотыФР_Z_Отчет(ПараметрыФР.ФР_Объект);
//	
//	Если НЕ ОборотыФР.Результат = Истина Тогда
//		Возврат;
//	КонецЕсли;	
//	
//	МенеджерЖурналРРО                				= РегистрыСведений.ЖурналZОтчетов.СоздатьМенеджерЗаписи();
//	
//	МенеджерЖурналРРО.ВидОбработки					= Константы.ТО_МодельФР.Получить();
//	
//	МенеджерЖурналРРО.ДатаСмены						= НачалоДня(ТекущаяДата());
//	МенеджерЖурналРРО.ФР_Номер_Z_Отчета				= ОборотыФР.ФР_Номер_Z_Отчета;
//	МенеджерЖурналРРО.ФР_ДатаПоследнегоОтчета		= ОборотыФР.ФР_ДатаПоследнегоОтчета;
//		
//	МенеджерЖурналРРО.ФР_ОборотиПродаж				= ОборотыФР.ФР_ОборотиПродаж;
//	МенеджерЖурналРРО.ФР_ОборотиПродаж_Нал			= ОборотыФР.ФР_ОборотиПродаж_Нал;
//	МенеджерЖурналРРО.ФР_ОборотиПродаж_БезНал		= ОборотыФР.ФР_ОборотиПродаж_БезНал;
//	МенеджерЖурналРРО.ФР_ОборотиПродаж_ПС			= ОборотыФР.ФР_ОборотиПродаж_ПС;
//	МенеджерЖурналРРО.ФР_ОборотиВозвратов			= ОборотыФР.ФР_ОборотиВозвратов;
//	МенеджерЖурналРРО.ФР_ОборотиВозвратов_Нал		= ОборотыФР.ФР_ОборотиВозвратов_Нал;
//	МенеджерЖурналРРО.ФР_ОборотиВозвратов_БезНал	= ОборотыФР.ФР_ОборотиВозвратов_БезНал;
//	МенеджерЖурналРРО.ФР_ОборотиВозвратов_ПС		= ОборотыФР.ФР_ОборотиВозвратов_ПС;
//	
//	МенеджерЖурналРРО.ФР_ОборотПродаж_А				= ОборотыФР.ФР_ОборотПродаж_А;
//	МенеджерЖурналРРО.ФР_ОборотПродаж_Б				= ОборотыФР.ФР_ОборотПродаж_Б;
//	МенеджерЖурналРРО.ФР_ОборотПродаж_В				= ОборотыФР.ФР_ОборотПродаж_В;
//	МенеджерЖурналРРО.ФР_ОборотПродаж_Г				= ОборотыФР.ФР_ОборотПродаж_Г;
//	МенеджерЖурналРРО.ФР_ОборотПродаж_Д				= ОборотыФР.ФР_ОборотПродаж_Д;
//	МенеджерЖурналРРО.ФР_ОборотПродаж_Е				= ОборотыФР.ФР_ОборотПродаж_Е;
//	
//	МенеджерЖурналРРО.ФР_ОборотВозврат_А			= ОборотыФР.ФР_ОборотВозврат_А;
//	МенеджерЖурналРРО.ФР_ОборотВозврат_Б			= ОборотыФР.ФР_ОборотВозврат_Б;
//	МенеджерЖурналРРО.ФР_ОборотВозврат_В			= ОборотыФР.ФР_ОборотВозврат_В;
//	МенеджерЖурналРРО.ФР_ОборотВозврат_Г			= ОборотыФР.ФР_ОборотВозврат_Г;
//	МенеджерЖурналРРО.ФР_ОборотВозврат_Д			= ОборотыФР.ФР_ОборотВозврат_Д;
//	МенеджерЖурналРРО.ФР_ОборотВозврат_Е			= ОборотыФР.ФР_ОборотВозврат_Е;
//	
//	МенеджерЖурналРРО.ФР_СтавкаНалога_А				= ОборотыФР.ФР_СтавкаНалога_А;
//	МенеджерЖурналРРО.ФР_СтавкаНалога_Б				= ОборотыФР.ФР_СтавкаНалога_Б;
//	МенеджерЖурналРРО.ФР_СтавкаНалога_В				= ОборотыФР.ФР_СтавкаНалога_В;
//	МенеджерЖурналРРО.ФР_СтавкаНалога_Г				= ОборотыФР.ФР_СтавкаНалога_Г;
//	МенеджерЖурналРРО.ФР_СтавкаНалога_Д				= ОборотыФР.ФР_СтавкаНалога_Д;
//	МенеджерЖурналРРО.ФР_СтавкаНалога_Е				= ОборотыФР.ФР_СтавкаНалога_Е;
//	
//	//Если ОборотыФР.ФР_СтавкаНалога_А <> 0 Тогда
//	МенеджерЖурналРРО.СуммаНДСПродаж_А = Окр(ОборотыФР.ФР_ОборотПродаж_А * ОборотыФР.ФР_СтавкаНалога_А / (100 + ОборотыФР.ФР_СтавкаНалога_А), 2);	
//	//КонецЕсли;	
//	МенеджерЖурналРРО.СуммаНДСПродаж_Б = Окр(ОборотыФР.ФР_ОборотПродаж_Б * ОборотыФР.ФР_СтавкаНалога_Б / (100 + ОборотыФР.ФР_СтавкаНалога_Б), 2);	
//	МенеджерЖурналРРО.СуммаНДСПродаж_В = Окр(ОборотыФР.ФР_ОборотПродаж_В * ОборотыФР.ФР_СтавкаНалога_В / (100 + ОборотыФР.ФР_СтавкаНалога_В), 2);	
//	МенеджерЖурналРРО.СуммаНДСПродаж_Г = Окр(ОборотыФР.ФР_ОборотПродаж_Г * ОборотыФР.ФР_СтавкаНалога_Г / (100 + ОборотыФР.ФР_СтавкаНалога_Г), 2);	
//	МенеджерЖурналРРО.СуммаНДСПродаж_Д = Окр(ОборотыФР.ФР_ОборотПродаж_Д * ОборотыФР.ФР_СтавкаНалога_Д / (100 + ОборотыФР.ФР_СтавкаНалога_Д), 2);	
//	МенеджерЖурналРРО.СуммаНДСПродаж_Е = Окр(ОборотыФР.ФР_ОборотПродаж_Е * ОборотыФР.ФР_СтавкаНалога_Е / (100 + ОборотыФР.ФР_СтавкаНалога_Е), 2);	
//	
//	//Если ОборотыФР.ФР_СтавкаНалога_А <> 0 Тогда
//	МенеджерЖурналРРО.СуммаНДСВозврат_А = Окр(ОборотыФР.ФР_ОборотВозврат_А * ОборотыФР.ФР_СтавкаНалога_А / (100 + ОборотыФР.ФР_СтавкаНалога_А), 2);	
//	//КонецЕсли;	
//	МенеджерЖурналРРО.СуммаНДСВозврат_Б = Окр(ОборотыФР.ФР_ОборотВозврат_Б * ОборотыФР.ФР_СтавкаНалога_Б / (100 + ОборотыФР.ФР_СтавкаНалога_Б), 2);	
//	МенеджерЖурналРРО.СуммаНДСВозврат_В = Окр(ОборотыФР.ФР_ОборотВозврат_В * ОборотыФР.ФР_СтавкаНалога_В / (100 + ОборотыФР.ФР_СтавкаНалога_В), 2);	
//	МенеджерЖурналРРО.СуммаНДСВозврат_Г = Окр(ОборотыФР.ФР_ОборотВозврат_Г * ОборотыФР.ФР_СтавкаНалога_Г / (100 + ОборотыФР.ФР_СтавкаНалога_Г), 2);	
//	МенеджерЖурналРРО.СуммаНДСВозврат_Д = Окр(ОборотыФР.ФР_ОборотВозврат_Д * ОборотыФР.ФР_СтавкаНалога_Д / (100 + ОборотыФР.ФР_СтавкаНалога_Д), 2);	
//	МенеджерЖурналРРО.СуммаНДСВозврат_Е = Окр(ОборотыФР.ФР_ОборотВозврат_Е * ОборотыФР.ФР_СтавкаНалога_Е / (100 + ОборотыФР.ФР_СтавкаНалога_Е), 2);	
//	
//	МенеджерЖурналРРО.ФР_СуммаСкидокПродажа			= ОборотыФР.ФР_СуммаСкидокПродажа;
//	МенеджерЖурналРРО.ФР_СуммаСкидокВозврат			= ОборотыФР.ФР_СуммаСкидокВозврат;
//	
//	МенеджерЖурналРРО.ФР_СуммаВнесение				= ОборотыФР.ФР_СуммаВнесение;
//	МенеджерЖурналРРО.ФР_СуммаИзъятие				= ОборотыФР.ФР_СуммаИзъятие;
//	
//	МенеджерЖурналРРО.ФР_СуммаВСейфе				= ОборотыФР.ФР_СуммаВСейфе;
//	
//	МенеджерЖурналРРО.КолЧековПродаж				= ОборотыФР.ФР_КоличествоЧеков_Продажа;
//	МенеджерЖурналРРО.КолЧековВозврата				= ОборотыФР.ФР_КоличествоЧеков_Возврат;
//	МенеджерЖурналРРО.КолАнулЧековПродаж			= ОборотыФР.ФР_КоличествоАннулированныхЧеков_Продажа;
//	МенеджерЖурналРРО.КолАнулЧековВозврата			= ОборотыФР.ФР_КоличествоАннулированныхЧеков_Возврат;
//	
//	Попытка
//		МенеджерЖурналРРО.Записать(Истина);
//		Логирование.ДобавитьЗаписьЖурнала(, "ЗаписатьОборотыФРВРегистрПередZОтчетом()", , Неопределено, Неопределено, "ОбщиеМодули.ТорговоеОборужование");
//	Исключение
//		Логирование.ДобавитьЗаписьЖурнала(УровеньЖурналаРегистрации.Ошибка, "ОШИБКА!!! ЗаписатьОборотыФРВРегистрПередZОтчетом()", ОписаниеОшибки(), Неопределено, Неопределено, "ОбщиеМодули.ТорговоеОборужование");
//	КонецПопытки;	
//	
//КонецПроцедуры	
...
Функция Подготовить_Рез_ОборотыФР() Экспорт
	...
	Рез_ОборотыФР.Вставить("ФР_ДатаПоследнегоОтчета",	Дата(1, 1, 1, 0, 0, 0)); 
...
Процедура ПолучитьZОтчетФР() Экспорт
	...
	ОборотыФР = ПолучитьОбработкуТО().ОборотыФР_Z_Отчет(ПараметрыФР.ФР_Объект, Рез_ОборотыФР);
		...
		Если Элемент0.ИмяУзла = "DAT" Тогда
			...
			Для Каждого Элемент1 Из Уровень1.ДочерниеУзлы Цикл
				Если Элемент1.ИмяУзла = "Z" Тогда
					Уровень2 = Элемент1;
					//это для фискализированной кассы
...
Процедура ЗаписатьВРегистрЖурналZОтчетов(ОборотыФР)
	...
	МенеджерЗаписиЖурналФР.ФР_ДатаПоследнегоОтчета		= ОборотыФР.ФР_ДатаПоследнегоОтчета;
...
Процедура ПечатьСостоянияМодема() Экспорт
	...
	Если НЕ ПолучитьОбработкуТО().ПечатьОтчетСостояниеМодема(ПараметрыФР) Тогда 
...
Процедура ОтправитьОтчетСостояниеМодема() Экспорт
	...
	СтруктураОтвета = ПолучитьОбработкуТО().ПолучитьСостояниеМодема(ПараметрыФР);
