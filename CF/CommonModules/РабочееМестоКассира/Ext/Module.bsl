...
Процедура ПаузаПередКомандой() Экспорт
	
	КоличествоСекунд = Константы.ФР_ПаузаПослеZОтчета.Получить();
	...
	ФормаИндикатор = ПолучитьОбщуюФорму("Индикатор");
	ФормаИндикатор.Заголовок = "Ожидание ФР...";
	ФормаИндикатор.ЭлементыФормы.Индикатор.МаксимальноеЗначение = КоличествоСекунд;
	ФормаИндикатор.Открыть();
	
	Для Сч = 0 По КоличествоСекунд Цикл
		
		КомандаWindows = "Timeout /T " + Формат(1, "ЧГ=0") + " /NoBreak";
		
		WshShell = Новый COMОбъект("WScript.Shell"); 
		WshShell.Run(КомандаWindows, 0, -1);
		ФормаИндикатор.Индикатор = ФормаИндикатор.Индикатор + 1;
