...
Процедура BAS_РОЗНИЦА_ОправитьSALE(ВсеЧекиЗаДень = Неопределено) Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
	  ...
	  //ТаблицаШапка.Колонки.Добавить("UUID_ЧекаККМ_ИзУФР", Новый ОписаниеТипов("Строка",,,, Новый КвалификаторыСтроки(СтрДлина(Новый УникальныйИдентификатор()))));
	  ////  Для Каждого СтрокаШапка Из ТаблицаШапка Цикл
	  //	ТаблицаШапка[0].UUID_ЧекаККМ_ИзУФР = СтрокаGUID;
	  ////  КонецЦикла;
		
		ТаблицаШапка 		= Новый ХранилищеЗначения(ОбщегоНазначения.ПолучитьШапкаЧекаККМ_ОтправитьНаСервер(ВыборкаДетальныеЗаписи.Ссылка), Новый СжатиеДанных(9));
		ТаблицаТовары 		= Новый ХранилищеЗначения(ОбщегоНазначения.ПолучитьТЧ_ТоварыЧекаККМ_ОтправитьНаСервер(ВыборкаДетальныеЗаписи.Ссылка), Новый СжатиеДанных(9));
		ТаблицаСкидки 		= Новый ХранилищеЗначения(ОбщегоНазначения.ПолучитьТЧ_ЖурналСкидокЧекаККМ_ОтправитьНаСервер(ВыборкаДетальныеЗаписи.Ссылка), Новый СжатиеДанных(9));
		ТаблицаДвижениеПС 	= Новый ХранилищеЗначения(ОбщегоНазначения.ПолучитьТЧ_ДвиженияПСЧекаККМ_ОтправитьНаСервер(ВыборкаДетальныеЗаписи.Ссылка), Новый СжатиеДанных(9));
		
		КодКассыККМ = Константы.Розница_КодКассы.Получить();
		РезультатСервиса = Прокси.ЗаписатьДокументЧекККМ(СокрЛП(КодКассыККМ), ТаблицаШапка, ТаблицаТовары, ТаблицаСкидки, ТаблицаДвижениеПС);
		
		Если РезультатСервиса.Результат Тогда
			
			ТаблицаРезультат = РезультатСервиса.ТаблицаЗаписиЭлементов.Получить();
			НовыйGUID = Новый УникальныйИдентификатор(ТаблицаРезультат[0].UUID_ЧекаККМ_ИзРозница);
			ОбъектЧекККМ = ВыборкаДетальныеЗаписи.Ссылка.ПолучитьОбъект();
			ОбъектЧекККМ.UUID_ЧекаККМ_ИзРозница = НовыйGUID;
			ОбъектЧекККМ.ОтправленНаСервер = РезультатСервиса.Результат;
			Попытка
