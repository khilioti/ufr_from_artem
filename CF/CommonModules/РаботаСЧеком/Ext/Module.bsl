...
Процедура ПараметрыЧекаККМ_ВидОперацииУстановить(ВидОперации) Экспорт 
	...
	Логирование.ДобавитьЗаписьЖурнала(, "ПараметрыЧекаККМ_ВидОперацииУстановить()", "Вид операции: " + ВидОперации, Неопределено, Неопределено, "ОбщийМодуль.РаботаСЧеком");
...
Процедура ПараметрыЧекаККМ_ВидОплатыУстановить(ВидОплаты) Экспорт 
	...
	Логирование.ДобавитьЗаписьЖурнала(, "ПараметрыЧекаККМ_ВидОплатыУстановить()", "Вид оплаты: " + ВидОплаты, Неопределено, Неопределено, "ОбщийМодуль.РаботаСЧеком");
...
Функция Отредактировать_Записать_ДокументЧекККМ(ПараметрыЧекаККМ, ПараметрыBPM7, ПараметрыFISHKA, Объект) Экспорт; 
	...
	Для Каждого СтрокаТЧ Из Объект.ТЧ_ЖурналСкидок Цикл 
		...
		// Дьяченко 290819
		НоваяСтрока.ID_акции 				= СтрокаТЧ.ID_акции;
		НоваяСтрока.АкционнаяСистема		= СтрокаТЧ.АкционнаяСистема;
		// Дьяченко
	КонецЦикла;	
...
Функция ДобавитьСтрокуВЧек(Объект, СтрокаДанных, Отказ = Ложь) Экспорт 
		
	глНомерПоследнегоЧека_VS = Константы.глСчетчикЧеков.Получить();	
		
	//Мулько 05.03.2020
	//проверим нет ли в чек товара с таким же код, но другой ценой
	МассивТоваровДляПроверки = Объект.Продажа.НайтиСтроки(Новый Структура("Код", СтрокаДанных.CODE));
	Если МассивТоваровДляПроверки.Количество() > 0 Тогда
		Для Каждого СтрокаМассива Из МассивТоваровДляПроверки Цикл
			Если СтрокаМассива.Цена <> СтрокаДанных.PRICE Тогда
				Отказ = Истина;
				ФормаПредупреждение = ПолучитьОбщуюФорму("ФормаПредупреждение");
				ФормаПредупреждение.ЭлементыФормы.НадписьСообщение.Заголовок = "Этот товар необходимо пробить в отдельном чеке!";
				ФормаПредупреждение.ОткрытьМодально();
				Возврат Ложь;
			КонецЕсли;
		КонецЦикла;	
	КонецЕсли;
...
Функция ПолучитьОтложенныйЧек() Экспорт 
	...
	Запрос.УстановитьПараметр("СостояниеЧекаККМ", Перечисления.СостояниеЧека.Отложенный);
...
Процедура ЗаполнитьПараметрыРМК(Объект, ДокументЧекККМ, ОчищатьПараметрыЧекаККМ = Истина) Экспорт  
	...
	Для Каждого СтрокаТЗ Из ТЗ_Товары Цикл
		...
		Если НЕ ОбщегоНазначения.ЭтоПодарочныйСертификат(СтрокаТЗ.Код) Тогда
	...
	РаботаСЧеком.Добавить_ПС_В_ТабличнуюЧасть(Объект);
