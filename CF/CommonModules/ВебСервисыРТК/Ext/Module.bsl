...
Процедура BAS_РОЗНИЦА_ОправитьSALE(ВсеЧекиЗаДень = Неопределено) Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ТаблицаДвыиденияПС = ОбщегоНазначения.ПолучитьТЧ_ДвиженияПСЧекаККМ_ОтправитьНаСервер(ВыборкаДетальныеЗаписи.Ссылка);
		ТаблицаДвыиденияПС.Колонки.Добавить("UUID", Новый ОписаниеТипов("Строка",,,, Новый КвалификаторыСтроки(СтрДлина(Новый УникальныйИдентификатор()))));
		ТаблицаДвыиденияПС.ЗаполнитьЗначения(UUID, "UUID");
		Если ТаблицаДвижениеПСОбщая.Количество() = 0 Тогда
			ТаблицаДвижениеПСОбщая = ТаблицаДвыиденияПС.Скопировать();
		Иначе
			ОбщегоНазначения.ЗагрузитьВТаблицуЗначений(ТаблицаДвыиденияПС, ТаблицаДвижениеПСОбщая);
		КонецЕсли;	
