...
Функция Подготовка_ТЧ_Товары(Объект, СтрокаРС, ДляОкругления10Коп = Ложь)
	...
	ТЧ_Товары = Объект.Продажа.Выгрузить().Скопировать(ОтборНеАнуляции, "Код, DATASTR1, Количество, НомерСтроки, НомерАкции, Цена, ЦенаБезСкидки, Акционный, Сумма, СкидкаБК, СкидкаСумма"); 
