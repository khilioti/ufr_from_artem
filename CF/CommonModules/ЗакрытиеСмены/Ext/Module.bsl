...
Процедура ОтправитьДанныеПоВебСервису() Экспорт
	...
	КонецПопытки;	
	Попытка
		ВебСервисыРТК.BAS_РОЗНИЦА_ОтправитьСведенияОТерминале();
	Исключение
		Логирование.ДобавитьЗаписьЖурнала(УровеньЖурналаРегистрации.Ошибка, "ВебСервисыРТК.BAS_РОЗНИЦА_ОтправитьСведенияОТерминале()", ОписаниеОшибки(), Неопределено, Неопределено, "ОбщийМодуль.ЗакрытиеСмены");
	КонецПопытки;	
...
Процедура ОтправитьДанныеЖурналаZОтчетовФР() Экспорт
	...
	Запрос.Текст = 
		...
		|	ЖурналZОтчетов.ФР_ДатаПоследнегоОтчета,
	...
	Попытка
		...
		Если РезультатСервиса.Результат Тогда
			Для Каждого СтрокаТЗ ИЗ ТЗОтчеты Цикл
				...
				//МенеджерЖурналZОтчетов.Отбор.ДатаСмены.Установить(СтрокаТЗ.ДатаСмены);
				//МенеджерЖурналZОтчетов.Отбор.ВидОбработки.Установить(СтрокаТЗ.ВидОбработки);
				//МенеджерЖурналZОтчетов.Отбор.ДатаСмены.Установить(СтрокаТЗ.ДатаСмены);
				//МенеджерЖурналZОтчетов.Отбор.ФР_СерийныйНомер.Установить(СтрокаТЗ.ФР_СерийныйНомер);
...
Процедура АнализРезультатаЗакрытияСмены(ПараметрыФР, ПризнакСозданияФайла_Z_Sale, ПризнакНаличияПродажТекущегоДня, ПризнакУcпехаFTP) Экспорт
	...
	ИначеЕсли ПризнакСозданияФайла_Z_Sale И ПризнакУcпехаFTP тогда
		...
		//--------------
		Попытка
			ПолучитьОбработкуТО().ЗакрытьПорт(ПараметрыФР);
