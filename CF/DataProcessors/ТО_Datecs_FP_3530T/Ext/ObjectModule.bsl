...
Функция ОткрытьПорт(ПараметрыФР) Экспорт
	...
	Исключение
		Возврат ЗакрытьПорт(ПараметрыФР);
	КонецПопытки;
...
Функция ПроверитьПодключениеФР(ПараметрыФР) Экспорт 
	...
	СерийныйНомерФР 														= СокрЛП(ПолучитьСерийныйНомер(ПараметрыФР)); 	
	Если СерийныйНомерФР<>СокрЛП(Константы.осн_КассаККМ.Получить()) Тогда		Возврат	Ложь    КонецЕсли;
	
	ДатаФР																	= ПолучитьДатуВремяФР(ПараметрыФР);
...
Функция ОборотыФР_Z_Отчет(пЭККР, Рез_ОборотыФР) Экспорт
	...
	Рез_ОборотыФР.ФР_ОборотиВозвратов_БезНал				= Число(пЭККР.SUMDAY(2,3,0,2,"000000"));
	Рез_ОборотыФР.ФР_ОборотиВозвратов_Нал					= Число(пЭККР.SUMDAY(2,0,0,2,"000000"));
	Рез_ОборотыФР.ФР_ОборотиВозвратов_ПС					= Число(пЭККР.SUMDAY(2,4,0,2,"000000"));
	Рез_ОборотыФР.ФР_ОборотиВозвратов_Ваучер				= Число(пЭККР.SUMDAY(2,5,0,2,"000000"));		
	...
	Рез_ОборотыФР.ФР_ОборотВозврат_А						= Число(пЭККР.SUMDAY(0,0,0,2,"000000"));
	Рез_ОборотыФР.ФР_ОборотВозврат_Б						= Число(пЭККР.SUMDAY(0,1,0,2,"000000"));
	Рез_ОборотыФР.ФР_ОборотВозврат_В						= Число(пЭККР.SUMDAY(0,2,0,2,"000000"));
	Рез_ОборотыФР.ФР_ОборотВозврат_Г						= Число(пЭККР.SUMDAY(0,3,0,2,"000000"));
	Рез_ОборотыФР.ФР_ОборотВозврат_Д						= Число(пЭККР.SUMDAY(0,4,0,2,"000000"));
...
Функция ПолучитьФискальныйНомер(ПараметрыФР) Экспорт
...
Функция Z_Отчет(ПараметрыФР) Экспорт
	...
	Если Константы.ТО_МодельФР.Получить()=3 И Найти(СерийныйНомерФР,"АТ")<>0 Тогда 
		Попытка
			ПечатьОтчетСостояниеМодема(ПараметрыФР);
		Исключение
		КонецПопытки;	
	КонецЕсли;	
...
//Мулько К.П.
Функция ПолучитьСостояниеМодема(ПараметрыФР) Экспорт 
...
Функция ПечатьОтчетСостояниеМодема(ПараметрыФР) Экспорт 
	пЭККР                                                                   = ПараметрыФР.ФР_Объект;
	пМодем                                                                  = ПараметрыФР.ФР_ОбъектМодем;
