...
//Мулько К.П.
Процедура BAS_РОЗНИЦА_ОтправитьСведенияОТерминале() Экспорт
		...
		Если НомерОбработки <> 0 Тогда
			...
			Если ПортОткрыт Тогда 
				Если ПараметрыPOSтерм <> Неопределено Тогда
					СтрокаИнфо = ПараметрыPOSтерм.POSтерминал_СтрокаИнфо;
				КонецЕсли;	
			КонецЕсли;		
		КонецЕсли;	
		BAS_РОЗНИЦА_ПередатьДанныеОТерминале(НомерОбработки, СтрокаИнфо);
	Исключение
...
Функция BAS_РОЗНИЦА_ПередатьДанныеОТерминале(КодБанка, СтрокаИнфо) Экспорт
	Прокси = ОбщегоНазначения.ПолучитьПрокси(Константы.Розница_Сервер.Получить(),"Store","Webservice19","http://www.dataforstore.prostor.ua","DataForStore");
	Возврат Прокси.ЗаписатьИнформациюОТерминале(Константы.Розница_КодКассы.Получить(), КодБанка, СтрокаИнфо);
КонецФункции	
