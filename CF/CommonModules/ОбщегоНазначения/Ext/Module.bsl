...
//Мулько 03.04.2020
Функция ЭтоУцененныйТовар(Штрихкод) Экспорт
	
	ШК = СокрЛП(Штрихкод);
	Если Лев(ШК, 4) = "2444" Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;	
...
//Мулько 03.04.2020
Функция ЕстьШтрихкодВТабличнойЧасти(Штрихкод, Объект) Экспорт
	
	ИмяКолонки = "Штрихкод";
	Если ТипЗнч(Объект) = Тип("ОбработкаОбъект.РМК") Тогда
		ИмяТабличнойЧасти = "Продажа";
	ИначеЕсли 	ТипЗнч(Объект) = Тип("ДокументОбъект.ЧекККМ") Тогда
		ИмяТабличнойЧасти = "ТЧ_Товары";
		ИмяКолонки = "ШК";
	Иначе
		Возврат Ложь;
	КонецЕсли;
	...
	Если ТабличнаяЧасть.Найти(Штрихкод, ИмяКолонки) = Неопределено Тогда
		Возврат Ложь;
	Иначе
		Возврат Истина;
	КонецЕсли;	
