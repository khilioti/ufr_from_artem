...
Функция фСкидкаНаТовар_ОкрДо10коп(Объект) Экспорт 
	...
	Если СКИДКА=0 Тогда 													Возврат Истина; 	КонецЕсли;
...
Функция Разложить_СкидкуБК_ПоВыделенымСтрокамЧека(Объект, ТаблицаОсновная, ОбщаяНаТоварСкидкаБК) Экспорт 
	...
	Если ТабВыполненияАкции.Количество()=0 Тогда  							Возврат 0  	КонецЕсли;
	Если СуммаОБЩАЯ=0 Тогда                             					Возврат 0  	КонецЕсли;
	
	  КоэфРаспределенияСкидки													= ОбщаяНаТоварСкидкаБК/СуммаОБЩАЯ; 		
	Если КоэфРаспределенияСкидки>1 Тогда 									Возврат 0 	КонецЕсли;
	...
	Для Каждого СтрокаТЧ Из МассивОтбор_ТаблицаОсновная Цикл 
		...
		НоваяСтрока.СкидкаСумма                     						= СтрокаЧека.СкидкаБК;
...
Процедура ДобавитьИзменитьСтроку_ТЧСкидки(Объект, ПарметрыАкции, Знач СуммаРазложенойСкидки=0, СтатусАкции) Экспорт 
	...
	Если МассивСтрок.Количество()>0 Тогда   
		...
		МассивСтрок[0].НаименованиеАкции									= ?(ПустаяСтрока(ПарметрыАкции.НаименованиеАкции), ПарметрыАкции.ТипАкции+" №"+ ПарметрыАкции.НомерАкции,  ПарметрыАкции.НаименованиеАкции);
		Если СуммаРазложенойСкидки>0 Тогда 
	...
	Иначе 	                                                 
		...
		СтрокаСкидки.НаименованиеАкции										= ?(ПустаяСтрока(ПарметрыАкции.НаименованиеАкции), ПарметрыАкции.ТипАкции+" №"+ ПарметрыАкции.НомерАкции,  ПарметрыАкции.НаименованиеАкции);
	...
	ИначеЕсли СкидкаПроцентГрВыдачи>0 Тогда   
    	Для Каждого СтрокаТЧ Из МассивТаблицаОсновная Цикл
			...
			Если СкидкаПроцентГрВыдачи = 100 Тогда 
				ОБЩАЯСкидка = ОБЩАЯСкидка + СтрокаТЧ.Сумма - 0.01;
			Иначе
				Если СтрокаРС_УА.СтрокаРС_УА.ВидЦеныДляОпределенияСкидкиНаГруппуВыдачи = Перечисления.ВидыЦенников.БелыйЦенник Тогда 
					СтруктураОтбора = Новый Структура("CODE, BAR", СтрокаТЧ.Код, СтрокаТЧ.Штрихкод);
