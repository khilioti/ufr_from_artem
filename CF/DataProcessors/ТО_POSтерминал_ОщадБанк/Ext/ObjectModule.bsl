...
Функция ОшибкаPOS_Сообщить(пPOSтерминал, ДопТекстОшибки="", ВыводПарметровПодключенияPOS=Ложь, НеВыводитьОшибку=Ложь)
    ...
    Логирование.ДобавитьЗаписьЖурнала(УровеньЖурналаРегистрации.Предупреждение, "ОшибкаPOS_Сообщить()", ПолныйТекст, Неопределено, Неопределено, "Обработки.ТО_POSтерминал_ОщадБанк");
...
Функция ПолучитьИнфоТерминала(ПараметрыPOSтерм) Экспорт
	...
	Если ПараметрыPOSтерм.POSтерминал_LastResult = 0 Тогда 
		...
		Логирование.ДобавитьЗаписьЖурнала(УровеньЖурналаРегистрации.Предупреждение, "ПолучитьИнфоТерминала()", ПараметрыPOSтерм.POSтерминал_СтрокаИнфо, Неопределено, Неопределено, "Обработки.ТО_POSтерминал_ОщадБанк");
        Возврат Истина;                                 
    КонецЕсли; 
	...
	Попытка
		...
		Попытка
			...
			Если ПараметрыPOSтерм.POSтерминал_LastResult=0 Тогда 
				...
				//Мулько К.П.
				//ПараметрыPOSтерм.POSтерминал_СтрокаИнфо = пPOSтерминал.TerminalInfo;
				Логирование.ДобавитьЗаписьЖурнала(УровеньЖурналаРегистрации.Предупреждение, "ОткрытьПорт()", "Порт открыт", Неопределено, Неопределено, "Обработки.ТО_POSтерминал_ОщадБанк");
	            Возврат Истина;                                 
	        КонецЕсли; 
...
Функция ЗакрытьПорт(ПараметрыPOSтерм) Экспорт 
	...
	Попытка		
		...
		Логирование.ДобавитьЗаписьЖурнала(УровеньЖурналаРегистрации.Предупреждение, "ЗакрытьПорт()", "Порт закрыт", Неопределено, Неопределено, "Обработки.ТО_POSтерминал_ОщадБанк");
	Исключение
	КонецПопытки;	
...
Функция ЗавершитьТранзакцию(ПараметрыPOSтерм, ПараметрыЧекаККМ) Экспорт
	...
	//Если LastResult<>0 Тогда 
		...
		Если СтрНайти(пPOSтерминал.Receipt, ">ВІДМІНА ОПЕРАЦІЇ<") <> 0 Тогда
			//ОшибкаPOS_Сообщить(пPOSтерминал, "ОШИБКА! Платеж не проведен! Будет АВТОМАТИЧЕСКИ сделан возврат по кассе.", Истина);
			ПолныйТекст = "ОШИБКА! Платеж не проведен! В квитанции терминала есть текст "">ВІДМІНА ОПЕРАЦІЇ<""";
...
Функция ПолучитьПарметрыЧекаТерминала(ПараметрыPOSтерм, ПараметрыЧекаККМ) Экспорт
	...
	Исключение
		Логирование.ДобавитьЗаписьЖурнала(УровеньЖурналаРегистрации.Ошибка, "ПолучитьПарметрыЧекаТерминала", "ОШИБКА ПРИ ПОЛУЧЕНИИ ПАРАМЕТРОВ ТРАНЗАКЦИИ ИЗ POS-Терминала!"+ОписаниеОшибки() , Неопределено, Неопределено, "Обработки.ТО_POSтерминал_ОщадБанк");
	КонецПопытки;
	...
	МассивТЕКСТ.Добавить("ОЩАДБАНК №: "+ПараметрыЧекаККМ.POSтерминал_ТерминалаID);
	...
	МассивТЕКСТ.Добавить("ЕПЗ: "+ПараметрыЧекаККМ.POSтерминал_ВидКарты);//27.03.2020 добавил ЕПЗ
	...
	Логирование.ДобавитьЗаписьЖурнала(, "ПолучитьПарметрыЧекаТерминала", " ПАРАМЕТРЫ ТРАНЗАКЦИИ => "+Текст , Неопределено, Неопределено, "Обработки.ТО_POSтерминал_ОщадБанк");
КонецФункции	
...
Функция АнуляцияТранзакции(ПараметрыPOSтерм) Экспорт
	...
	//Если Не ВвестиЧисло(НомерЧека, "Введите номер ТЕРМИНАЛЬНОГО ЧЕКА !!!", 10, 0) И НомерЧека>0 Тогда
   	Если Не ВвестиЧисло(НомерЧека, "Введите номер ТЕРМИНАЛЬНОГО ЧЕКА !!!", 12, 0) И НомерЧека>0 Тогда
...
Функция PrintLastSettleCopy_На_ФР(ПараметрыPOSтерм, МассивНефискальногоТекста) Экспорт 
	...
	Попытка
		...
		Если LastResult=0 Тогда 
			...
			МассивНефискальногоТекста.Добавить("----ОЩАДБАНК----");
			МассивНефискальногоТекста.Добавить("                ");
...
Функция Печать_X_Отчет_На_ФР(ПараметрыPOSтерм, МассивНефискальногоТекста) Экспорт
	...
	Попытка
		...
		Если LastResult=0 Тогда //24.02.2020
	
			ТекстСообщения = пPOSтерминал.Receipt;
			
			//МассивНефискальногоТекста	= ОбщегоНазначения.РазложитьСтрокуВМассивПодстрок(ТекстСообщения, Символы.ПС);
			Массив = ОбщегоНазначения.РазложитьСтрокуВМассивПодстрок(ТекстСообщения, Символы.ПС);
			СтрокиВывода 														= "";
			СтрокаВывода														= "";
